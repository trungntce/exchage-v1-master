                ...
                <c:when test="${sellDTOList.size() > 0}">
                  <c:forEach items="${sellDTOList}" var="item" varStatus="loop">
                    <tr class="table-success">
                      ...
                      <td>
                        <c:if test="${item.state == 4 && item.tradeState == 2 && item.refSellState == 4}">
                            <button class="btn-confirm-txid" data-id="${item.sellId}"><spring:message code="common.button.confirm.txid.title"/></button>
                        </c:if>
                        <c:if test="${item.state == 4 && item.tradeState == 4 && item.refSellState == 4}">
                            <form method="POST" action="/admin/sell/safe/finish">
...
<script>
  ...
  function searchInit(){
    location.href="/admin/sell/safe/list";
  }
  $(".btn-confirm-txid").click(function(){
      const id = $(this).data("id")
      $(`#modal-sell-confirm-txid [name="sellId"]`).val(id)
      $(`#modal-sell-confirm-txid`).modal("show")
  })
</script>
</body>
</html>