        ...
        <div id="main" class="panel panel-inverse">
            <div class="panel-heading panel-top">
                <h4 class="panel-title">
                    <spring:message code="admin.subtitle.buy.mytrade.list.title"/>
                </h4>
                <!-- RIBBON -->
            ...
            <div class="panel-body">
                ...
                <div class="note note-gray-500">
                    <div class="note-content">
                        <form name="search" id="search" method="GET" class="input-group input-group-sm">
                          ...
                          <select name="viewRole" class="form-select">
                            <option value="" <c:if test="${search.viewRole == ''}"> selected </c:if>><spring:message code="search.name.all.title"/></option>
                            <c:if test="${wallet.role == 'CENTRAL_BANK' || wallet.role == 'OPERATOR'}">
                              <option value="MASTER" <c:if test="${search.viewRole == 'MASTER'}"> selected </c:if>>MASTER</option>
                            </c:if>
                            <c:if test="${wallet.role == 'CLIENT' || wallet.role == 'OPERATOR'}">
                              <option value="SITE_ADMIN" <c:if test="${search.viewRole == 'SITE_ADMIN'}"> selected </c:if>>SITE_ADMIN</option>
                            </c:if>
                            <c:if test="${wallet.role == 'OPERATOR'}">
                              <option value="TRADER" <c:if test="${search.viewRole == 'TRADER'}"> selected </c:if>>TRADER</option>
                            </c:if>
                          </select>
                ...
                <div class="text-end">
                    <a class="btn btn-primary btn-sm me-1" href="${_ctx}/admin/buy/mytrade/write"><spring:message code="button.buy.add.title"/></a>
                </div>
                    ...
                    <div class="table-responsive mb-3">
                        <table class="table table-bordered table-hover text-nowrap align-middle mb-0 table-sm">
                            ...
                            <tr>
                                ...
                                <th rowspan="2"><spring:message code="common.admin.field.action.title"/></th>
                            </tr>
                            <tr>
                                ...
                                <c:when test="${buyDTOList.size() > 0}">
                                    <c:forEach items="${buyDTOList}" var="item" varStatus="loop">
                                        <tr class="table-success">
                                            ...
                                            <td class="tx-hash" title="${item.buyerWalletAddress}">${item.buyerWalletAddress}</label></td>
                                            <td>${item.buyerName}</td>
                                            <td class="tx-hash" title="${item.sellerWalletAddress}">${item.sellerWalletAddress}</label></td>
                                            ...
                                            <td>
                                                <c:if test="${item.sellerWalletAddress eq wallet.walletAddress && item.state == 2}">
                                                    <form method="POST" action="/admin/buy/mytrade/finish">
                                                        <input type="hidden" name="buyId" value="${item.buyId}">
                                                        <button><spring:message code="common.button.finish.title"/></button>
                                                    </form>
                                                </c:if>
                                            </td>
                                        </tr>
