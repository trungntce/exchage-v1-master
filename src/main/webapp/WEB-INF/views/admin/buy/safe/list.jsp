                    ...
                    <div class="table-responsive mb-3">
                        <table class="table table-bordered table-hover text-nowrap align-middle mb-0 table-sm">
                            ...
                            <c:choose>
                                <c:when test="${buyDTOList.size() > 0}">
                                    <c:forEach items="${buyDTOList}" var="item" varStatus="loop">
                                        <tr class="table-success">
                                            ...
                                            <td>
                                                <c:if test="${item.state == 4 && item.tradeState == 4 && item.refBuyState == 4}">
                                                    <button class="btn-confirm-txid" data-id="${item.buyId}"><spring:message code="common.button.confirm.txid.title"/></button>
                                                </c:if>
                                                <c:if test="${item.state == 2 && item.tradeState == 4 && item.refBuyState == 4}">
                                                    <form method="POST" action="/admin/buy/safe/finish">
                                                        <input type="hidden" name="buyId" value="${item.buyId}">
                                                        <button type="submit"><spring:message code="common.button.cashDeposit.title"/></button>
                                                    </form>
                                                </c:if>
                                            </td>
                                        </tr>
                                ...
                                <c:otherwise>
                                    <tr>
                                        <td colspan="20"><spring:message code="common.message.noData.title"/></td>
                                    </tr>
                                </c:otherwise>
                            </c:choose>
                            </tbody>
...
<%@ include file="/WEB-INF/layout/admin/modals/confirm_txid.jsp"%>
<script>
    function submitForm(target){
        $(target).submit();
    }
    function searchInit(){
        location.href="/admin/buy/safe/list";
    }

    $(".btn-confirm-txid").click(function(){
        const id = $(this).data("id")
        $(`#modal-buy-confirm-txid [name="buyId"]`).val(id)
        $(`#modal-buy-confirm-txid`).modal("show")
    })
</script>
</body>
</html>