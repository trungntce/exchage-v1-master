...
<c:if test="${buyRequest.symbol == 'BANI' || buyRequest.symbol == 'EGG'}">
...
<div class="wrapper">
    ...
    <section class="wrap trade-wrap">
       ...
       <!--  <article class="sub-title sub-title--trade">
            ...
            <p><spring:message code="buy.register.page.content"/></p>
        </article> -->


        <div class="container">
            <form method="post" id="formBuyRequest" action="${baseUrl}/buyRequest">
                ...
                <div class="form-full" id="balance-trade">
                    ...
                    <div class="form-wrap form-full">
                            <select name="symbol" class="symbol" target="#buyRequest">
                                <c:forEach items="${tokens}" var="item" varStatus="loop">
                                    <c:choose>                                    
                                        <c:when test="${not empty buyRequest.symbol && item.symbol == buyRequest.symbol}">
                                            <option value="${item.symbol}" unitPrice="${item.unitPrice}" selected>
                                                ${item.symbol}
                                            </option>
                                        </c:when>
                                        <c:otherwise>
                                            <option value="${item.symbol}" unitPrice="${item.unitPrice}">
                                                ${item.symbol}
                                            </option>
                                        </c:otherwise>
                                    </c:choose>
                    ...
                    <div class="form-wrap form-full">                        
                        <select id="walletAddress" name="buyerWalletAddress" class="walletSelect" target="#buyRequest" style="display:${ownerSymbol}">
                    ...
                    <c:if test="${wallet != null && fn:containsIgnoreCase('CLIENT', roleCode)}">
                        <div class="form-wrap form-full">
                            <select name="type" target="#buyRequest">
                    ...
                    <div class="form-wrap form-full">
                        <spring:message code="buy.register.form.tokenNum.hint" var="tokenNumHint"/>
                        <input name="quantity" placeholder="${tokenNumHint}" class="tokenNum" type="number" target="#buyRequest" min="1"/>
                    </div>

                    <div class="trade-pop-form-info">
                        <dl class="trade-pop-form-info-item">
                            <dt><spring:message code="buy.register.form.price.title"/></dt>  
                            <dd class="price-one" style="display:${ownerSymbol}">
                              <c:forEach items="${tokens}" var="item" varStatus="loop" end="0"><fmt:formatNumber type="number" maxFractionDigits="3" value="${item.unitPrice}"/> <spring:message code="common.unit.won.title"/>
                              </c:forEach>
                            </dd>   
                            <dd class="other-price-one" style="display:${otherSymbol}">
                                <spring:message code="trade.enter.price" var="enterPriceHint"/>
                                <input name="unitPrice" class="input-other-price onlynum" type="text" maxFractionDigits="3" placeholder="${enterPriceHint}" class="tokenNum onlynum" maxlength="32" target="#buyRequest"/> <spring:message code="common.unit.won.title"/>
                            </dd>
                        </dl>                      
                        <dl class="trade-pop-form-info-item ex-height-auto">
                            <dt class="ex-height-auto"><spring:message code="buy.register.form.tokenCnt.title"/></dt>
                            <dd><label class="lb-token-cnt">0 </label> <spring:message code="common.unit.count.title"/></dd>
                        </dl>
                         <dl class="trade-pop-form-info-item ex-height-auto">
                            <dt class="ex-height-auto"><spring:message code="buy.register.form.fee.title"/></dt>
                            <dd><label class="lb-fee">0 </label></dd>
                        </dl>
                        <dl class="trade-pop-form-info-item ex-height-auto">
                            <dt class="ex-height-auto"><spring:message code="buy.register.form.amount.title"/></dt>
                            <dd>
                        ...
                        <dl class="trade-pop-form-info-item ex-height-auto">
                            <dt class="ex-height-auto"><spring:message code="buy.register.form.totalPrice.title"/></dt>
                    ...
                    <div class="join-terms">
                        <div class="title"><input type="checkbox" id="cb-term-all"><spring:message code="buy.register.form.termAll.title"/></div>
                        <div class="title"><input type="checkbox" class="cb-term"><label onclick="javascript:showModal('.modal-otc-1', false)"><spring:message code="buy.register.form.term1.title"/></label></div>
                        <div class="title"><input type="checkbox" class="cb-term"><label onclick="javascript:showModal('.modal-otc-2', false)"><spring:message code="buy.register.form.term2.title"/></label></div>
                    </div>

                    <div class="start-login">
                        <div class="btn btn-primary btn-submit" target="#buyRequest" style="pointer-events: none; background-color: #6a4d78"><spring:message code="common.button.submit.title"/></div>
                        <div class="btn secondary" onclick="btnBuyBack()"><spring:message code="common.button.goList.title"/></div>
                    </div>
...
<script> 
    ...
    $(".walletSelect").change(async function(){
        ...
        var url = changeSearchParam({buyerWalletAddress: wallet, walletAddress:wallet})
    ...
    $(".tokenNum").keyup(function(){      
        ...
        if (tokenSelect === 'BANI' || tokenSelect == 'EGG') { 
            ...
            // var amountFee = (val * dataset.fee * dataset.unitPrice)/100;
            var amount = val * dataset.unitPrice;
            var totalPrice = amountFee + amount;
    ...
    function elementDisplay(tokenSelect){    
        ...
        if (tokenSelect === 'BANI' || tokenSelect == 'EGG') {       
            ...
            $('.trade-warning-title-other').css("display","none");
            ...
            $('.trade-warning-title-owner').css("display","none");
        }
    }
