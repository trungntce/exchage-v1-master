...
//				particlesJS.load(id, '/js/particles-1.json', function () {});
			});
		}, 500);
	};
	...
	* ---------------------------*/
	...
	function formatOptions(state) {
		...
		var $state = $(
			'<span ><img class="woox-icon" sytle="display: inline-block;" src="resources/img/web/' + state.element.value.toLowerCase() + '.png" /> ' + state.text + '</span>'
		);
		return $state;
	}
	...
	* --------------------- */
	...
	CRUMINA.chartStatistics = function () {
		...
		$.each(names, function (i, item) {
			...
			$.getJSON('resources/json/' + item.name.toLowerCase() + '-c.json', function (data) {
		...
		init: function () {
			...
			for (var key in this.maps) {
				...
				data.markers.forEach(function (item) {
					data.config.icon['iconUrl'] = '/resources/img/web/' + item.icon;
