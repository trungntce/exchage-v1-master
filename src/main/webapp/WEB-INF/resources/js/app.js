...
var handleMobileSidebarToggle = function() {
	...
	$(document).on('click touchstart', function(e) {
		...
		if (!e.isPropagationStopped() && sidebarProgress !== true) {
			...
			if ($(window).width() <= 767) {
				if ($('#app').hasClass('page-right-sidebar-toggled')) {
					sidebarProgress = true;
					$('#app').removeClass('page-right-sidebar-toggled');
				}
			}
		}
	});
	...
	$(document).on('click', '[data-click=sidebar-toggled]', function(e) {
		e.stopPropagation();
		var sidebarClass = 'app-sidebar-mobile-toggled';
		var targetContainer = '#app';
...
var handleSidebarMinify = function() {
	...
	if (Cookies) {
		...
		if (sidebarMinified == 'true') {
			$('#app').addClass('page-sidebar-minified');
		}
	}
};
...
var handleThemePageStructureControl = function() {
	...
	$(document).on('change', '.theme-panel [name="sidebar-gradient"]', function() {
		...
		if ($(this).is(':checked')) {
			$('#app').addClass('gradient-enabled');
			sidebarGradient = true;
		} else {
			$('#app').removeClass('gradient-enabled');
		}
  ...
  // SIDEBAR - sidebar fixed selection
	$(document).on('change', '.theme-panel [name="sidebar-fixed"]', function() {
		...
		if ($(this).is(':checked')) {
			if (!$('.theme-panel [name="header-fixed"]').is(':checked')) {
				...
				$('#app').addClass('page-header-fixed');
			}
			$('#app').addClass('page-sidebar-fixed');
			if (!$('#app').hasClass('page-sidebar-minified')) {
				generateSlimScroll($('.sidebar [data-scrollbar="true"]'));
			}
			sidebarFixed = true;
		} else {
			$('#app').removeClass('page-sidebar-fixed');
			if ($('.sidebar .slimScrollDiv').length !== 0) {
				if ($(window).width() <= 979) {
					$('.sidebar').each(function() {
						if (!($('#app').hasClass('page-with-two-sidebar') && $(this).hasClass('sidebar-right'))) {
			...
			if ($('#app .sidebar-bg').length === 0) {
				$('#app').append('<div class="sidebar-bg"></div>');
			}
		}
	...
	$(document).on('change', '.theme-panel [name="header-fixed"]', function() {
		...
		if ($(this).is(':checked')) {
			...
			$('#app').removeClass('page-header-fixed');
		}
		if (Cookies) {
		...
		mouseenter: function() {
			if ($('#app').hasClass('page-sidebar-minified')) {
				...
				if (targetMenuHtml) {
					...
					var sidebarX = (!$('#app').hasClass('page-with-right-sidebar') && $('body').css('direction') != 'rtl') ? (sidebarOffset.left + sidebarWidth) : ($(window).width() - sidebarOffset.left);
					var targetHeight = $(targetMenu).height();
					var targetOffset = $(this).offset();
					var targetTop = targetOffset.top - $(window).scrollTop();
					var targetLeft = (!$('#app').hasClass('page-with-right-sidebar') && $('body').css('direction') != 'rtl') ? sidebarX : 'auto';
					var targetRight = (!$('#app').hasClass('page-with-right-sidebar') && $('body').css('direction') != 'rtl') ? 'auto' : sidebarX;
					var windowHeight = $(window).height();
...
var handleAjaxMode = function(setting) {
	...
	function checkSidebarActive(url) {
		...
		if ($(targetElm).length !== 0) {
			...
			$(targetElm).parents().addClass('app-sidebar-mobile-toggled');
		}
	}
...
var handleSetPageOption = function(option) {
	if (option.pageContentFullHeight) {
		$('#app').addClass('page-content-full-height');
	}
	if (option.pageSidebarLight) {
		$('#app').addClass('page-with-light-sidebar');
	}
	if (option.pageSidebarRight) {
		$('#app').addClass('page-with-right-sidebar');
	}
	if (option.pageSidebarWide) {
		$('#app').addClass('page-with-wide-sidebar');
	}
...
var handleClearPageOption = function(option) {
	if (option.pageContentFullHeight) {
		$('#app').removeClass('page-content-full-height');
	}
	if (option.pageSidebarLight) {
		$('#app').removeClass('page-with-light-sidebar');
	}
	if (option.pageSidebarRight) {
		$('#app').removeClass('page-with-right-sidebar');
	}
	if (option.pageSidebarWide) {
		$('#app').removeClass('page-with-wide-sidebar');
	}
	if (option.pageSidebarMinified) {
		$('#app').removeClass('page-sidebar-minified');
	}
	if (option.pageSidebarTransparent) {
